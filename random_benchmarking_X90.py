...
x_opt_payload = np.load(f'data/payloads/x90_gate/x90_opt_res.npz', allow_pickle=True)['x_opt_res'].item()
x_opt_payload_NN = np.load(f'data/payloads/x90_gate/x90_opt_res_NN.npz', allow_pickle=True)['x_opt_res_NN'].item()
x_opt_payload_nm = np.load(f'data/payloads/x90_gate/x90_opt_res_nm.npz', allow_pickle=True)['x_opt_res_nm'].item()
x_opt_payload_nm_NN = np.load(f'data/payloads/x90_gate/x90_opt_res_nm_NN.npz', allow_pickle=True)['x_opt_res_nm_NN'].item()
...
for i, payload in enumerate(x_opt_payload_list):
    ...
    x_bench = run_sequence_benchmark(system_params=system_params, collapse_ops=collapse_ops,
                                    optimized_payload=payload, gate_name='x90_gate', 
    ...
    plt.ylim(0,1.02)
    plt.title(f'X90 gate sequence fidelity decay for payload {i}')
    plt.legend()
    plt.tight_layout()
    plt.show()
