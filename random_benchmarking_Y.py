...
y_opt_payload = np.load(f'data/payloads/y_gate/y_opt_res.npz', allow_pickle=True)['y_opt_res'].item()
y_opt_payload_NN = np.load(f'data/payloads/y_gate/y_opt_res_NN.npz', allow_pickle=True)['y_opt_res_NN'].item()
y_opt_payload_nm = np.load(f'data/payloads/y_gate/y_opt_res_nm.npz', allow_pickle=True)['y_opt_res_nm'].item()
y_opt_payload_nm_NN = np.load(f'data/payloads/y_gate/y_opt_res_nm_NN.npz', allow_pickle=True)['y_opt_res_nm_NN'].item()

y_opt_payload_list = [y_opt_payload, y_opt_payload_NN, y_opt_payload_nm, y_opt_payload_nm_NN]

for i, payload in enumerate(y_opt_payload_list):
    print(f'\nBenchmarking result for payload {i}:')
    y_bench = run_sequence_benchmark(system_params=system_params, collapse_ops=collapse_ops,
                                    optimized_payload=payload, gate_name='y_gate', 
                                    sequence_lengths=np.arange(1,1000,1), save=True)
    print(benchmarking_summary(y_bench['scan_result'], y_bench['fit_params']))

    m = y_bench['scan_result']['m']
    Fm = y_bench['scan_result']['avg_fidelity']
    A = y_bench['fit_params']['A']
    p = y_bench['fit_params']['p']
    B = y_bench['fit_params']['B']
    ...
    plt.ylim(0,1.02)
    plt.title(f'Y gate sequence fidelity decay for payload {i}')
    plt.legend()
    plt.tight_layout()
    plt.show()

    print('\nSaved benchmark artifacts:' if 'saved_paths' in y_bench else 'Benchmark not saved.')
    if 'saved_paths' in y_bench:
        pprint(y_bench['saved_paths'])
