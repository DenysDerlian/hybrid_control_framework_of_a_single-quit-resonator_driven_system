...
y90_opt_payload = np.load(f'data/payloads/y90_gate/y90_opt_res.npz', allow_pickle=True)['y90_opt_res'].item()
y90_opt_payload_NN = np.load(f'data/payloads/y90_gate/y90_opt_res_NN.npz', allow_pickle=True)['y90_opt_res_NN'].item()
y90_opt_payload_NNf1 = np.load(f'data/payloads/y90_gate/y90_opt_res_NNf1.npz', allow_pickle=True)['y90_opt_res_NN'].item()
y90_opt_payload_nm = np.load(f'data/payloads/y90_gate/y90_opt_res_nm.npz', allow_pickle=True)['y90_opt_res_nm'].item()
y90_opt_payload_nm_NN = np.load(f'data/payloads/y90_gate/y90_opt_res_nm_NN.npz', allow_pickle=True)['y90_opt_res_nm_NN'].item()
y90_opt_payload_nm_NNf1 = np.load(f'data/payloads/y90_gate/y90_opt_res_nm_NNf1.npz', allow_pickle=True)['y90_opt_res_nm_NN'].item()

y90_opt_payload_list = [y90_opt_payload, y90_opt_payload_NN, y90_opt_payload_NNf1,
                         y90_opt_payload_nm, y90_opt_payload_nm_NN, y90_opt_payload_nm_NNf1]

for i, payload in enumerate(y90_opt_payload_list):
    print(f'\nBenchmarking result for payload {i}:')
    y90_bench = run_sequence_benchmark(system_params=system_params, collapse_ops=collapse_ops,
                                    optimized_payload=payload, gate_name='y90_gate', 
                                    sequence_lengths=np.arange(1,1000,1), save=True)
    print(benchmarking_summary(y90_bench['scan_result'], y90_bench['fit_params']))

    m = y90_bench['scan_result']['m']
    Fm = y90_bench['scan_result']['avg_fidelity']
    A = y90_bench['fit_params']['A']
    p = y90_bench['fit_params']['p']
    B = y90_bench['fit_params']['B']
    ...
    plt.ylim(0,1.02)
    plt.title(f'Y90 gate sequence fidelity decay for payload {i}')
    plt.legend()
    plt.tight_layout()
    plt.show()

    print('\nSaved benchmark artifacts:' if 'saved_paths' in y90_bench else 'Benchmark not saved.')
    if 'saved_paths' in y90_bench:
        pprint(y90_bench['saved_paths'])
